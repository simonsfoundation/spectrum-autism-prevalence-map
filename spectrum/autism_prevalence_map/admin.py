                ...
                for row in reader:
                    ...
                    try:
                        ...
                        if yearpublished_number:
                            updated_values = {
                                'yearpublished': row['Year published'] if 'Year published' in row and row['Year published'] is not None else '',
                                'authors': row['Authors'] if 'Authors' in row and row['Authors'] is not None else '', 
                                'country': row['Country'] if 'Country' in row and row['Country'] is not None else '', 
                                'area': row['Area'] if 'Area' in row and row['Area'] is not None else '', 
                                'samplesize': row['Sample size'] if 'Sample size' in row and row['Sample size'] is not None else '', 
                                'age': row['Age (years)'] if 'Age (years)' in row and row['Age (years)'] is not None else '', 
                                'individualswithautism': row['Individuals with autism'] if 'Individuals with autism' in row and row['Individuals with autism'] is not None else '', 
                                'diagnosticcriteria': row['Diagnostic criteria'] if 'Diagnostic criteria' in row and row['Diagnostic criteria'] is not None else '', 
                                'diagnostictools': row['Diagnostic tools'] if 'Diagnostic tools' in row and row['Diagnostic tools'] is not None else '', 
                                'percentwaverageiq': row['Percent w/ average IQ'] if 'Percent w/ average IQ' in row and row['Percent w/ average IQ'] is not None else '', 
                                'sexratiomf': row['Sex ratio (M:F)'] if 'Sex ratio (M:F)' in row and row['Sex ratio (M:F)'] is not None else '', 
                                'prevalenceper10000': row['Prevalence (per 10,000)'] if 'Prevalence (per 10,000)' in row and row['Prevalence (per 10,000)'] is not None else '', 
                                'confidenceinterval': row['95% Confidence interval'] if '95% Confidence interval' in row and row['95% Confidence interval'] is not None else '', 
                                'categoryadpddorasd': row['Category (AD, PDD or ASD)'] if 'Category (AD, PDD or ASD)' in row and row['Category (AD, PDD or ASD)'] is not None else '',
                                'yearsstudied': row['Year(s) studied'] if 'Year(s) studied' in row and row['Year(s) studied'] is not None else '',
                                'recommended': row['Recommended'] if 'Recommended' in row and row['Recommended'] is not None else '', 
                                'studytype': row['Study type'] if 'Study type' in row and row['Study type'] is not None else '',
                                'meanincomeofparticipants': row['Mean income of participants'] if 'Mean income of participants' in row and row['Mean income of participants'] is not None else '',
                                'educationlevelofparticipants': row['Education level of participants'] if 'Education level of participants' in row and row['Education level of participants'] is not None else '',
                                'citation': row['Citation'] if 'Citation' in row and row['Citation'] is not None else '',
                                'link1title': row['Link 1 Title'] if 'Link 1 Title' in row and row['Link 1 Title'] is not None else '',
                                'link1url': row['Link 1 URL'] if 'Link 1 URL' in row and row['Link 1 URL'] is not None else '',
                                'link2title': row['Link 2 Title'] if 'Link 2 Title' in row and row['Link 2 Title'] is not None else '',
                                'link2url': row['Link 2 URL'] if 'Link 2 URL' in row and row['Link 2 URL'] is not None else '',
                                'link3title': row['Link 3 Title'] if 'Link 3 Title' in row and row['Link 3 Title'] is not None else '',
                                'link3url': row['Link 3 URL'] if 'Link 3 URL' in row and row['Link 3 URL'] is not None else '',
                                'link4title': row['Link 4 Title'] if 'Link 4 Title' in row and row['Link 4 Title'] is not None else '',
                                'link4url': row['Link 4 URL'] if 'Link 4 URL' in row and row['Link 4 URL'] is not None else ''
                            }
